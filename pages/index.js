...
export default function GiftHelperApp() {
  ...
  return (
    <div className="min-h-screen p-4 bg-gradient-to-b from-red-100 via-white to-green-100 flex flex-col items-center font-sans relative overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row w-full max-w-6xl gap-6 z-10">
            ...
            <motion.div key={person.id} whileHover={{ scale: 1.05, rotate: 1 }} className="cursor-pointer">
              <Card className="rounded-3xl shadow-2xl border-2 border-green-400 bg-white/95 transition-transform duration-300" onClick={() => setExpandedPerson(expandedPerson === person.id ? null : person.id)}>
                <CardContent className="p-4 flex flex-col items-center">
                  ...
                  <p className="text-sm md:text-md text-gray-600">Tap to {expandedPerson === person.id ? 'hide' : 'view'} gift ideas 🎄</p>
                </CardContent>
              </Card>
        ...
        <div className="w-full md:w-80 p-4 bg-white/90 rounded-xl shadow-lg border-2 border-red-400 flex flex-col gap-4">
          <Button className="bg-red-600 text-white hover:bg-red-700" onClick={() => setBudgetOpen(!budgetOpen)}>
            {budgetOpen ? 'Hide Budget Options' : 'Set a budget for your gift list?'}
          </Button>
          {budgetOpen && (
            <input type="number" min={0} placeholder="Enter amount in USD" className="p-2 border rounded" value={budget} onChange={(e) => setBudget(Number(e.target.value))} />
          )}

          {giftList.length > 0 && (
            <div className="flex flex-col gap-2 max-h-96 overflow-y-auto">
              <h4 className="font-semibold text-red-700">Shopping List</h4>
              {giftList.map((item, index) => (
                <div key={index} className="flex flex-col border p-2 rounded bg-white/80">
                  <div className="flex justify-between items-center mb-1">
                    <span>{item.text}</span>
                    <Button className="bg-red-600 text-white hover:bg-red-700 text-sm px-2 py-1 rounded" onClick={() => removeFromGiftList(index)}>Remove</Button>
                  </div>
                  <textarea placeholder="Add a note..." value={item.notes} onChange={(e) => updateNote(index, e.target.value)} className="p-1 border rounded text-sm w-full"></textarea>
                </div>
              ))}
              <Button className="bg-green-700 text-white hover:bg-green-800 mt-2" onClick={downloadPDF}>Download Gift List PDF</Button>
            </div>
          )}
        </div>
      </div>
      ...
      <style>{`
        ...
        .animate-snowflake { animation: snowflake linear infinite; animation-duration: 15s; }
